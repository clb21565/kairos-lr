##### kairos derep-detect | lightweight id of shared gene clusters in metagenomic assemblies
##### author: connor brown 

import argparse

parser=argparse.ArgumentParser(description='| kairos derep-detect | dereplicate contigs based on open reading frames and generate dereplicated overlaps')
...
parser.add_argument("--coords_file",type=str,required=True,help="Input ORF coordinates from bash to integrate into cluster df")
parser.add_argument("--out_prefix", type=str,default="DerepDetect", required=False,help="output file prefix")
parser.add_argument("--contigMD",type=str,help="samtools faidx of the contig fasta file",required=True)
...
coords=pd.read_csv(args.coords_file,sep="\t",header=None,names=["orf","start","end","strand","metadata"])
contigMD=pd.read_csv(args.contigMD,sep="\t",header=None,names=["contig","length","offset","linebases","linewidth"])
contigMD=contigMD[["contig","length"]]
...
overlap_orf_df_t1=deduplicated_overlaps_OUT[["contig1","identical_orf_position_contig1"]].drop_duplicates()
overlap_orf_df_t1.columns=["contig","orf"]
overlap_orf_df_t2=deduplicated_overlaps_OUT[["contig2","identical_orf_position_contig2"]].drop_duplicates()
overlap_orf_df_t2.columns=["contig","orf"]

overlap_orf_df=pd.concat([overlap_orf_df_t1,overlap_orf_df_t2])
overlap_orf_df["orf"]=overlap_orf_df["contig"]+"_"+overlap_orf_df["orf"].astype(str)

merged=pd.merge(overlap_orf_df,coords,left_on="orf",right_on="orf")
merged=pd.merge(contigMD,merged,left_on="contig",right_on="contig")
merged.to_csv("{}_deduplicated_overlap_orfs_coords.tsv".format(args.out_prefix),index=False,sep="\t")

merged[["contig","start","end","strand","orf"]].drop_duplicates().to_csv("{}_potential_hgts.bed".format(args.out_prefix),index=False,sep="\t",header=False)
